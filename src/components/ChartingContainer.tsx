            ...
            <Grid container direction='row' justify='flex-start' alignItems='center' spacing={3}>
                {props.actives.map((activeMetric: Metric) => (
                    <Grid item key={props.metrics.indexOf(activeMetric) || -1}>
                        {activeMetric.name.toLocaleUpperCase()} : {activeMetric.historicalValues[activeMetric.historicalValues.length - 1].value || -1} {activeMetric.unit}
                        <LineChart width={400} height={400} data={activeMetric.historicalValues.map(entry => {
                            ...
                            <Tooltip />
                            <Legend />
                            <Line name={activeMetric.name.toLocaleUpperCase() + ' ' + activeMetric.unit} type='monotone' dataKey='value' stroke='#0000ff' yAxisId={0} />
                        </LineChart>
                    </Grid>
                ))}
            </Grid>
